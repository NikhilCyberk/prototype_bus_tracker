...
// // NewBusHandler creates a new bus handler
// func NewBusHandler(db *gorm.DB, broadcast chan models.Message, mapsService *services.MapsService) *BusHandler {
// 	return &BusHandler{
// 		DB:          db,
// 		Broadcast:   broadcast,
...
func (h *BusHandler) UpdateBusLocation(c *gin.Context) {
	...
	var bus models.Bus
	if err := h.DB.Preload("Route").First(&bus, id).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "Bus not found"})
		return
	}
	...
	// Calculate ETA for next stop if route exists
	if bus.Route != nil {
		nextStop, err := h.getNextStop(bus)
		if err == nil {
			eta, _ := h.MapsService.CalculateETA(
				bus.Latitude,
				bus.Longitude,
				nextStop.Latitude,
				nextStop.Longitude,
...
func (h *BusHandler) getNextStop(bus models.Bus) (*models.BusStop, error) {
	var stops []models.BusStop
	if err := h.DB.Where("route_id = ?", bus.RouteID).Order("order ASC").Find(&stops).Error; err != nil {
		return nil, err
	}
	...
	// Find the next stop (simple implementation - could be enhanced)
	for _, stop := range stops {
		if stop.Order > bus.LastStopOrder {
			return &stop, nil
		}
	}
